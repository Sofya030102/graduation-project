...
echo Или оставьте поле пустым и нажмите ENTER для значения по умолчанию (C:\golosovanie\genesis.json)

set /P new_gen=""

echo Введите полный путь к рабочей папке geth и нажмите ENTER
echo Или оставьте поле пустым и нажмите ENTER для значения по умолчанию (C:\golosovanie\accounts)
...
IF NOT "%new_gen%" == "" (
	IF NOT "%new_path%" == "" (
		...
		geth init "%new_gen%" --datadir "C:\golosovanie\accounts"
		geth --rpc -rpcaddr "127.0.0.1" --rpcapi "net, web3, personal, miner, eth, admin" --networkid 15 --datadir "C:\golosovanie\accounts" --allow-insecure-unlock --rpccorsdomain "*" console
	)
) ELSE (
	IF NOT "%new_path%" == "" (
		geth init "C:\golosovanie\genesis.json" --datadir "%new_path%"
		geth --rpc -rpcaddr "127.0.0.1" --rpcapi "net, web3, personal, miner, eth, admin" --networkid 15 --datadir "%new_path%" --allow-insecure-unlock --rpccorsdomain "*" console
	) ELSE (
		geth init "C:\golosovanie\genesis.json" --datadir "C:\golosovanie\accounts"
		geth --rpc -rpcaddr "127.0.0.1" --rpcapi "net, web3, personal, miner, eth, admin" --networkid 15 --datadir "C:\golosovanie\accounts" --allow-insecure-unlock --rpccorsdomain "*" console
	)
)
